...
export class SubscriptionsController {
  ...
  async getActiveSubscription(@Req() req: Request) {
    ...
    if (!userPayload.tenantId) {
      throw new ForbiddenException(
        'El usuario administrador del SaaS no posee suscripción institucional',
      );
    }
  ...
  @Post('change')
  @UseGuards(AuthGuard)
  async changePlan(@Req() req: Request, @Body() body: { planId: string }) {
    const userPayload = (req as any).user;

    // Solo OWNER o SUPER_ADMIN pueden cambiar planes
    if (userPayload.role !== 'OWNER' && userPayload.role !== 'SUPER_ADMIN') {
      throw new ForbiddenException(
        'Solo el propietario del instituto puede cambiar el plan de suscripción',
      );
    }

    if (!userPayload.tenantId) {
      throw new ForbiddenException(
        'El usuario administrador del SaaS no puede modificar planes de institutos',
      );
    }
